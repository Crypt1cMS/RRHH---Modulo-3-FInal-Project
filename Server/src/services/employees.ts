...
const createEmployee = async (employeeData: any) => {
    const { full_name, email,  cedula, price_per_hour, role, status } = employeeData;

    const query = `
        INSERT INTO public.employees (full_name, email,  cedula, price_per_hour, role, status)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING *;
    `;

    try {
        const result = await pool.query(query, [full_name, email,  cedula, price_per_hour, role, status]);
        return result.rows[0];
    } catch (err) {
        const error = err as Error;
...
const findEmployeeByEmail = async (email: string) => {
    ...
    try {
        ...
        return result.rows[0]; // Return the first matched employee, or undefined if none found
    } catch (err) {
        const error = err as Error;
        throw new Error(`Unable to fetch employee: ${error.message}`);
    }
